...
const KEY_FILE_PATH = util_1.resolvePath('certs/dev.key');
const CERT_FILE_PATH = util_1.resolvePath('certs/dev.pem');
class Mkcert {
    constructor(options) {
        this.getLatestHash = () => __awaiter(this, void 0, void 0, function* () {
            return {
                key: yield util_1.getHash(KEY_FILE_PATH),
                cert: yield util_1.getHash(CERT_FILE_PATH)
            };
        });
        ...
        this.mkcertSavedPath = util_1.resolvePath(process.platform === 'win32' ? 'mkcert.exe' : 'mkcert');
    ...
    createCertificate(hostnames) {
        return __awaiter(this, void 0, void 0, function* () {
            const hostlist = hostnames.join(' ');
            const mkcertBinnary = yield this.getMkcertBinnary();
            if (!mkcertBinnary) {
                logger_1.debug(`Mkcert does not exist, unable to generate certificate for ${hostlist}`);
            }
            yield util_1.ensureDirExist(KEY_FILE_PATH);
            yield util_1.ensureDirExist(CERT_FILE_PATH);
            const cmd = `${mkcertBinnary} -install -key-file ${KEY_FILE_PATH} -cert-file ${CERT_FILE_PATH} ${hostlist}`;
            yield util_1.exec(cmd);
    ...
    initMkcert() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            logger_1.debug('The mkcert does not exist, download it now');
            if (!sourceInfo) {
    ...
    upgradeMkcert() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!versionInfo.shouldUpdate) {
                logger_1.debug('Mkcert is kept latest version, update skipped');
                return;
            }
            if (versionInfo.breakingChange) {
                logger_1.debug('The current version of mkcert is %s, and the latest version is %s, there may be some breaking changes, update skipped', versionInfo.currentVersion, versionInfo.nextVersion);
                return;
            }
            logger_1.debug('The current version of mkcert is %s, and the latest version is %s, mkcert will be updated', versionInfo.currentVersion, versionInfo.nextVersion);
            yield this.downloadMkcert(sourceInfo.downloadUrl, this.mkcertSavedPath);
    ...
    renew(hosts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!record.contains(hosts)) {
                logger_1.debug(`The hosts changed from [${record.getHosts()}] to [${hosts}], start regenerate certificate`);
                yield this.regenerate(record, hosts);
                return;
            }
            const hash = yield this.getLatestHash();
            if (record.tamper(hash)) {
                logger_1.debug(`The hash changed from ${util_1.prettyLog(record.getHash())} to ${util_1.prettyLog(hash)}, start regenerate certificate`);
                yield this.regenerate(record, hosts);
                return;
            }
            logger_1.debug('Neither hosts nor hash has changed, skip regenerate certificate');
        });
    }
    /**
     * Get certificates
